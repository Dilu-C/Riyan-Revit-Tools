    ...
    def BtnGenerate_Click(self, sender, e):
        ...
        try:
            ...
            with DB.Transaction(doc, "Generate Legends & Sheets") as t:
                ...
                default_elev_height_ft = 3800.0 / 304.8
                try:
                    for p in template_table.Parameters:
                        if p.Definition and p.StorageType == DB.StorageType.Double:
                            if p.Definition.Name.strip().lower() == "elevation height":
                                if p.HasValue:
                                    default_elev_height_ft = p.AsDouble()
                                break
                except: pass
                ...
                def is_tall(el):
                    try:
                        ...
                        if p and p.AsDouble() > (default_elev_height_ft - (300.0 / 304.8)):
                            return 1
                    except: pass
                    return 0
                ...
                if global_max_door_height_ft > (default_elev_height_ft - (300.0 / 304.8)):
                    tall_elev_height_ft = global_max_door_height_ft + (800.0 / 304.8)
                    tall_delta_height = tall_elev_height_ft - default_elev_height_ft
                    tall_table_height_ft = default_table_height_ft + tall_delta_height
                else:
                    tall_delta_height = 0.0
                    tall_table_height_ft = default_table_height_ft
                ...
                for t_id, model_el in sorted_items:
                    ...
                    if is_tall_door:
                        ...
                        is_empty_sheet = (current_x < 0.001 and row_count == 0)
                        is_tall_row = (row_count == 0 and current_row_max_height > default_table_height_ft + 0.1)
                        if not (is_empty_sheet or is_tall_row):
                            is_first_in_row = False
                            row_width_used = MAX_ROW_WIDTH + 100.0
                            current_row_max_height = default_table_height_ft * 2.0
                    else:
                    ...
                    try:
                        ...
                        templates_to_delete.update(current_template_ids)
                        ...
                        if delta_height > 0.01:
                            ...
                            bottom_elements_to_move = []
                            for cid in copied_ids:
                                if new_table and cid == new_table.Id: continue
                                if new_plan_comp and cid == new_plan_comp.Id: continue
                                if new_elev_comp and cid == new_elev_comp.Id: continue
                                if elevation_text_note and cid == elevation_text_note.Id: continue
                                
                                c_el = doc.GetElement(cid)
                                if not c_el: continue
                                
                                box = c_el.get_BoundingBox(current_view)
                                if box:
                                    if box.Min.Y < template_elev_bottom_y + current_y + (500.0/304.8):
                                        bottom_elements_to_move.append(cid)
                                        
                            if bottom_elements_to_move:
                                from System.Collections.Generic import List
                                DB.ElementTransformUtils.MoveElements(doc, List[DB.ElementId](bottom_elements_to_move), DB.XYZ(0, -delta_height, 0))
                        ...
                        if new_table:
                            try:
                                ...
                                try:
                                    ...
                                    for p in new_table.Parameters:
                                        if p.Definition and p.StorageType == DB.StorageType.Double and not p.IsReadOnly:
                                            p_name = p.Definition.Name.strip().lower()
                                            if p_name == "height":
                                                h_param = p
                                            elif p_name == "elevation height":
                                                eh_param = p
                                                
                                    if h_param: h_param.Set(t_height_ft)
                                    if eh_param: eh_param.Set(default_elev_height_ft + delta_height)
                                    
                                    if delta_height > 0.01:
                                        doc.Regenerate()
                                        new_tbox = new_table.get_BoundingBox(current_view)
                                        if new_tbox:
                                            dy_table = (template_table_top_y + current_y) - new_tbox.Max.Y
                                            if abs(dy_table) > 0.001:
                                                DB.ElementTransformUtils.MoveElement(doc, new_table.Id, DB.XYZ(0, dy_table, 0))
                                except: pass
                        ...
                        if new_elev_comp:
                            try:
                                ...
                                target_ffl = template_table_top_y + current_y + ffl_gap - delta_height
                ...
                # Delete the template elements since we now always copy them!
                for eid in templates_to_delete:
                    try: doc.Delete(eid)
                    except: pass
